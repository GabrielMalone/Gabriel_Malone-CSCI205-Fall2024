...
public:
	...
	bool cyclesPresent(){
		...
		unordered_map<string, bool> yet_detected_codes;
		for (auto v_code: detectedCycles){
			int index = 1;
			string final_path = "";
		...
		for (auto it = yet_detected_codes.begin(); it != yet_detected_codes.end(); ++it) {
			if (it->first.length() > 1)
			cout << it->first.substr(0, it->first.length()-2) << endl;
		}
		cout << Colors::RESET;
		printMatrix();
		cout << endl;
